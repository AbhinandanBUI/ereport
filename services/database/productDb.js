...
exports.filterproduct = async (data) => {
  try {
    ...
    console.log("this is result here",result)
    return result;
  } catch (e) {
    throw new Error("Unable to create filter: " + e.message);
  }
...
exports.WeeklyStats = async (data) => {
  try {
   const w1 = await tproduct.aggregate([
    ...
    week: { $week: "$dop" }}},
    { $match : { userId: { $in: [data.userId] },  "year":2022,"week":data.weekId,"dayofweek":1} },
    { $group: { _id: 1, total: { $sum: "$amount" } } }
  ]);
  const w2 = await tproduct.aggregate([
    { $project: {dop: "$dop",amount:"$amount",userId:"$userId",year: { $year: "$dop" }, dayofweek:{ $dayOfWeek:  "$dop"  },
    week: { $week: "$dop" }}},
    { $match : { userId: { $in: [data.userId] },  "year":2022,"week":data.weekId,"dayofweek":2} },
    { $group: { _id: 2, total: { $sum: "$amount" } } }
  ]);
  const w3 = await tproduct.aggregate([
    { $project: {dop: "$dop",amount:"$amount",userId:"$userId",year: { $year: "$dop" }, dayofweek:{ $dayOfWeek:  "$dop"  },
    week: { $week: "$dop" }}},
    { $match : { userId: { $in: [data.userId] },  "year":2022,"week":data.weekId,"dayofweek":3} },
    { $group: { _id: 3, total: { $sum: "$amount" } } }
  ]);
  const w4 = await tproduct.aggregate([
    { $project: {dop: "$dop",amount:"$amount",userId:"$userId",year: { $year: "$dop" }, dayofweek:{ $dayOfWeek:  "$dop"  },
    week: { $week: "$dop" }}},
    { $match : { userId: { $in: [data.userId] },  "year":2022,"week":data.weekId,"dayofweek":4} },
    { $group: { _id: 4, total: { $sum: "$amount" } } }
  ]);
  const w5 = await tproduct.aggregate([
    { $project: {dop: "$dop",amount:"$amount",userId:"$userId",year: { $year: "$dop" }, dayofweek:{ $dayOfWeek:  "$dop"  },
    week: { $week: "$dop" }}},
    { $match : { userId: { $in: [data.userId] },  "year":2022,"week":data.weekId,"dayofweek":5} },
    { $group: { _id: 5, total: { $sum: "$amount" } } }
  ]);
  const w6 = await tproduct.aggregate([
    { $project: {dop: "$dop",amount:"$amount",userId:"$userId",year: { $year: "$dop" }, dayofweek:{ $dayOfWeek:  "$dop"  },
    week: { $week: "$dop" }}},
    { $match : { userId: { $in: [data.userId] },  "year":2022,"week":data.weekId,"dayofweek":6} },
    { $group: { _id: 6, total: { $sum: "$amount" } } }
  ]);
  const w7 = await tproduct.aggregate([
    { $project: {dop: "$dop",amount:"$amount",userId:"$userId",year: { $year: "$dop" }, dayofweek:{ $dayOfWeek:  "$dop"  },
    week: { $week: "$dop" }}},
    { $match : { userId: { $in: [data.userId] },  "year":2022,"week":data.weekId,"dayofweek":7} },
